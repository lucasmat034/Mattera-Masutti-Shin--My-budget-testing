...
@given(parsers.parse('un budget de {amount:f} € pour la catégorie "{category}" du {start} au {end}'))
...
@given(parsers.parse('des dépenses existantes de {amount:f} € en "{category}"'))
def add_existing_spending(context, transaction_service, amount, category):
    ...
    context['initial_spending'] = amount


# ===== WHEN steps =====

@when(parsers.parse('j\'ajoute une nouvelle dépense de {amount:f} € en "{category}"'))
...
@then(parsers.parse('le total des dépenses est de {expected:f} €'))
...
@then(parsers.parse('le budget est dépassé de {amount:f} €'))
...
@then(parsers.parse('l\'alerte indique un dépassement de {percentage:f}%'))
